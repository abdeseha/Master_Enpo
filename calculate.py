...
class Characterize:
    ...
    def __init__(self, operations={}, temp_opts={}, in_opt=False, last_time=0, operations_num=1):
        ...
        for i,dive in enumerate(dives):
            if  time[shift_time+i] >= self.last_time:
                ...
                if dive > 5 and self.in_opt == False:
                    ...
                    self.operation =  {'nb_of_use':1, 'stab_value':0, 'start_time':np.array([0]), 'end_time':np.array([0]), 'max':np.array([0])}
                    self.in_opt = False                    
                     
        self.last_time = time.iloc[-1]

        return self.operations ,self.operation, self.in_opt, self.last_time, self.operations_num